...
| Reflexive-push durability rule (`2026-07-02-reflexive-push-durability-rule.md`) | **Implemented** (2026-07-04, wrap session) — items 1+3: session-end ritual now says commit **then push** (template CLAUDE.md + session-lifecycle wrapup checklist) and `cold-start-checks.sh` reports unpushed commits ([WARN] above 5). Item 2's steward-summary variant deferred: additive S1-STEWARD output change → 2.1 ceremony, and cold-start placement serves the intent (you discover the debt at the start of the session that can fix it). |
| Context as scarce judgment-bearing resource (`2026-07-04-context-as-scarce-resource-delegate-browser-reserve-main-for-judgment.md`) | **Watchlisted** (1 vote, tak-tdf peer session) — delegate browser/high-volume execution to subagents, reserve the main thread for coordination + nuanced decisions. Promote opportunistically on the next `practices/multi-agent-orchestration.md` or `agents/FANOUT_PATTERN.md` touch (it extends the existing when-NOT-to-fan-out table with a when-TO-delegate dual). |
